...
while total_attempts < simulations:
    ...
    #
    #
    snowdin_time = snowdin_strat
    print("Enter Box Road",ftm(snowdin_time))
    SEED = ['Icecap Jerry', 'Icecap Jerry', 'Icecap Jerry', 'Icecap Jerry', 'Snowdrake Jerry', 'Snowdrake Jerry', 'Snowdrake Jerry', 'Snowdrake Jerry']
    kills = 0
    #Scripted snodrake
    steps = scr_steps(120, 30, 16)
    snowdin_time += encounter_times["Snowdrake"] + encountering_time + steps + savepointinteract + equip_glove
    print("Exitting Snowdrake", ftm(snowdin_time))
    encounter = SEED[0] # Scripted at dogi
    kills = 3
    snowdin_time += snowding_rock_to_dogi
    print("Enter Dogi", ftm(snowdin_time))
    ...
    if encounter == "Icecap Jerry":
        ...
        snowdin_time += encounter_times[encounter]
        kills += 2
    print("Exit First Random", ftm(snowdin_time))
    snowdin_time += dogi_to_greater
    print("Enter Greater Dog", ftm(snowdin_time))
    snowdin_time += greater_dog_fight - (dog_skip_save * (2)) # Greater Dog Fight
    print("Exit Greater Dog", ftm(snowdin_time))
    snowdin_time += deadly_bridge
    print("Enter Snowdin", ftm(snowdin_time))
    SEEDING = 0
    while kills < 16: # Bridge grind
        ...
        if encounter == 'Snowdrake Jerry':
            ...
            if (kills-1) == 14:
                kills += 1
                snowdin_time += jerry_kill
        print(kills, "Kills (right after killing it)", ftm(snowdin_time))
    snowdin_time += exit_snowdin
    print("Snowdin", ftm(snowdin_time))
